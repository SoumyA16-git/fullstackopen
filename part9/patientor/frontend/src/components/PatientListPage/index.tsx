...
import type { PatientFormValues, Patient } from '../../types.ts';
...
const PatientListPage = ({ patients, setPatients }: Props) => {
    ...
    try {
      ...
      if (axios.isAxiosError(e)) {
        if (e.response?.data && typeof e.response.data === 'object' && 'error' in e.response.data) {
          const errData = e.response.data as { error: string | Array<{ message: string }> };
  ...
  return (
    <div className="App">
      <Box>
        <Typography align="center" variant="h6" style={{ marginTop: '1em', marginBottom: '1em' }}>
          Patient list
        </Typography>
      </Box>
